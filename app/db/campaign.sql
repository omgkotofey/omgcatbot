CREATE TABLE IF NOT EXISTS `campaign` (
  ...
  `user_id` bigint COMMENT 'Unique identifier for campaign user or bot',
  `is_follower` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'User already followed us on Twitter',
  `twitter_link` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'Received from user Twitter link',
  `has_retweet` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'User''s twitter link is really retweet of owr''s twitter account tweet',
  `ethereum address` CHAR(50) COMMENT 'User''s ethereum address',
  `has_tokens_earned` tinyint(1) DEFAULT NULL COMMENT 'Has user earned any tokens from us',
